...
aipw = function(df, eta)
{
  ...
  chem = ifelse(df$age < eta[1], 1, 0) 
  amp = ifelse(df$diam < eta[2], 1, 0)
  policy = rep(0, 272)
  ...
  data.policy = df
  data.policy$treatment = as.factor(i) # overwrites the treatment variable with new treatment decision
